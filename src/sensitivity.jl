...
""" Function to plot sensitivity graph """
    function plot_tariff_sensitivity(param_path::String, study_name::String, tariff_in_analisys::String, 
        ...
        tariff_ticks    = collect(1:1:length(step_id))

        for p in optimparam.peaksid, con in transparam.busnames
            ...
            plot(contract, color = :blue,label = "Contract", linewidth=2.5, xticks = (tariff_ticks,step_id))

            if optimparam.flextolflag == 1
                plot!(gen_tol_up, color = :red,label = "Penalty limits")
                plot!(gen_tol_down, color = :red,label = "")
            end

            plot!(fixed_tol_up, color = :gray,label = "Fixed tolerance", linewidth=3.0)
            plot!(fixed_tol_down, color = :gray,label = "", linewidth=3.0)
            hline!([maximum(busdata[con][p].f[p][(year-1)*dimensions.nmonths+month,:])], color = :black,label = "Max, min and q50 demand", linewidth=1.5, linestyle=:dash)
            hline!([minimum(busdata[con][p].f[p][(year-1)*dimensions.nmonths+month,:])], color = :black,label = "", linewidth=1.5, linestyle=:dash)
            hline!([quantile(busdata[con][p].f[p][(year-1)*dimensions.nmonths+month,:],0.5)], color = :black,label = "", linewidth=1.5, linestyle=:dash)
...
function plot_risk_sensitivity(param_path::String, study_name::String, 
    ...
    for p in optimparam.peaksid, con in transparam.busnames
        ...
        # =========================================
            ...
            # plot!(contract, color = :blue,label = "Contract", xticks = step_id, linewidth=2.5, yticks = yticks_base, ylims =ylims_base)
            plot(contract, color = :blue,label = "Contract", linewidth=2.5, xticks = (param_ticks,step_id))
